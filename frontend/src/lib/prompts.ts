const ROLE_PROMPTS: Record<string, { base: string }> = {
  analyst: {
...
export const CHAIRPERSON_PROMPT = `You are the CHAIRPERSON of this council. You did NOT participate in the debate.
...
export function formatTranscript(responses: RoundResponse[], roundNum: number): string {
  const lines = [`--- ROUND ${roundNum} ---`];
